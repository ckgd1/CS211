...
int main()
{
  ...
  if(win == true)
    cout << "You won!" << "You answered " << numCorrect << "questions." << endl;
...
expr makeQuestion()
{
  ...
  if(e.oprt == MULT) //if the operator is multiplication, make operands between 1 and 20 for the first operand and between 1 and 10 for the second operand.
    ...
    {
      e.oprd1 = (op)(rand() % 20) + 1; //?????????; //create a random number between 1 and 20
      e.oprd2 = (op)(rand() % 10) + 1;//?????????;//create a random number between 1 and 10
    }
